...
const TOKEN_KEY = "customer-app:token";
  ...
  const value = useMemo<AuthContextValue>(
    () => ({
      ...
      async loginWithGoogle() {
        ...
        const { token: newToken, user: googleUser } = await api.googleAuth({ idToken, role: "CUSTOMER" });
        await AsyncStorage.setItem(TOKEN_KEY, newToken);
        api.setToken(newToken);
        setUser(googleUser);
        setToken(newToken);
      ...
      async register(data) {
        const { token: newToken, user: newUser } = await api.register({ ...data, role: "CUSTOMER" });
        await AsyncStorage.setItem(TOKEN_KEY, newToken);
        api.setToken(newToken);
        setUser(newUser);
        setToken(newToken);
