...
const TOKEN_KEY = "rider-app:token";
  ...
  const value = useMemo<AuthContextValue>(
    () => ({
      ...
      async loginWithGoogle() {
        ...
        const { token: newToken, user: googleUser } = await api.googleAuth({ idToken, role: "RIDER" });
        await AsyncStorage.setItem(TOKEN_KEY, newToken);
        api.setToken(newToken);
        setUser(googleUser);
        setToken(newToken);
      ...
      async register(data) {
        const { token: newToken, user: newUser } = await api.register({ ...data, role: "RIDER" });
        await AsyncStorage.setItem(TOKEN_KEY, newToken);
        api.setToken(newToken);
        setUser(newUser);
        setToken(newToken);
